...
# LIGO-specific readligo.py and utils.py 
...
if whiten_data:
    ...
    strain_H1_whiten = lg.whiten(strain_H1,psd_H1,dt)
...
template_p_whiten = lg.whiten(template_p,psd_H1,dt)
template_c_whiten = lg.whiten(template_c,psd_H1,dt)
...
for det in dets:
    ...
    if make_plots:
        ...
        lg.plot(time, timemax, SNR, pcolor, det, eventname, tevent, strain_whitenbp, template_match,
...
# write the files:
lg.write_wavfile(eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
...
template_p_smooth = lg.whiten(template_p,psd_smooth,dt)
...
lg.write_wavfile(eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
strain_H1_shifted = lg.reqshift(strain_H1_whitenbp,fshift=fshift,sample_rate=fs)
strain_L1_shifted = lg.reqshift(strain_L1_whitenbp,fshift=fshift,sample_rate=fs)

# write the files:
lg.write_wavfile(eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
lg.write_wavfile(eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = lg.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
lg.write_wavfile(eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
...
if data_segments:
    ...
    strain, time, chan_dict = lg.loaddata(fn_L1, 'H1')
    ...
    segment_list = lg.dq_channel_to_seglist(chan_dict[DQflag])
    ...
    segment_list = lg.dq_channel_to_seglist(chan_dict['NO_CBC_HW_INJ'])
    print('Number of segments with DQflag',DQflag,' = ',len(segment_list))
    iseg = 0
    for segment in segment_list:
        time_seg = time[segment]
